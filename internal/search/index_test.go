...
func TestSearch(t *testing.T) {
	tests := []struct {
		...
		query      string
		max        int
		wantCount  int // -1 means: don't assert count, only first
		wantFirst  string
		firstField string // "title" or "source"; empty skips first-result check
	}{
		{
			name: "build and rank by title",
			chunks: []Chunk{
			...
			query: "kubernetes", max: 10,
			wantCount: -1, wantFirst: "Kubernetes Pods", firstField: "title",
		},
		{
			name:      "no results",
			chunks:    []Chunk{{Source: "a.md", Title: "Hello", Body: "World"}},
			query:     "nonexistent",
			max:       10,
			wantCount: 0,
		},
		{
			...
			query: "controller", max: 10,
			wantCount: -1, wantFirst: "Tagged Doc", firstField: "title",
		},
		{
			name: "title boost",
			chunks: []Chunk{
			...
			query: "ingress", max: 10,
			wantCount: -1, wantFirst: "Ingress Configuration", firstField: "title",
		},
		{
			...
			chunks: func() []Chunk {
				var c []Chunk
				for range 20 {
					c = append(c, Chunk{Source: "x.md", Title: "Topic", Body: "keyword content"})
				}
				return c
			}(),
			query: "keyword", max: 5,
		...
		{
			name: "stem matches across query",
			chunks: []Chunk{
				{Source: "a.md", Title: "Controllers", Body: "controllers reconcile resources"},
				{Source: "b.md", Title: "Other", Body: "unrelated content here"},
			},
			query: "controller", max: 10,
			wantCount: -1, wantFirst: "a.md", firstField: "source",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			idx := NewIndex(tt.chunks)
			results := idx.Search(tt.query, tt.max)
			...
			if tt.firstField != "" {
				...
				var got string
				switch tt.firstField {
				case "title":
					got = results[0].Chunk.Title
				case "source":
					got = results[0].Chunk.Source
				}
				if got != tt.wantFirst {
					t.Fatalf("first %s = %q; want %q", tt.firstField, got, tt.wantFirst)
				}
			}
		})
	}
